...
`issues_create` accepts optional `assigneeIdentifiers` and `reporterIdentifier` values using active member UUIDs returned by `workspace_members`. Multiple assignees are supported; `issues_update` replaces the full assignee set, and an empty array clears it. The legacy singular `assigneeIdentifier` remains accepted for compatibility. The reporter defaults to the token owner; only an admin may select another member or change an existing reporter. A selected reporter is stored as the issue creator and automatically subscribed, while the token owner remains the actor shown in the activity feed.
